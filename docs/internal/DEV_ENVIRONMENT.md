...
| `clickhouse-connect` for ClickHouse | **Done.** `rowproof/connectors/_chwire.py` talks the real driver; the old stdlib-`urllib` client is gone. `tests/integration/test_clickhouse_real.py` runs for real (no longer skipped) against the ClickHouse container `docker-compose.yml` starts — see docs/internal/LAPTOP_SETUP.md. |
