...
It uses the same hash-and-bisect idea as [reladiff](https://github.com/erezsh/reladiff) (excellent project, actively maintained, successor to data-diff) — split the key range into segments, hash each segment's normalised rows in one query per side, bisect the segments that disagree. Where it differs: every cross-engine type difference is governed by a published rule table and cited in the output, and the output is a sign-off report rather than a diff count. (I'm also considering a hosted version on top — scheduled runs, history, alerts — but that's just an idea right now, not something that exists.)
